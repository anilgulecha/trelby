...
class MyNavigator(wx.VListBox):
    ...
    # resizeFunc - function that takes single argument - sets our width
    def __init__(self, parent, id, width, getCfgGui, resizeFunc):
        wx.VListBox.__init__(self, parent, id, size = (width, -1),
                style = wx.NO_BORDER)
        self.items = []
        self.currentLine = 0
    ...
    def OnLeftUp(self, event):
        if self.dragStarted:
            ...
            changedW = w - x + self.dragPos
            if changedW < MIN_NAVIGATOR_WIDTH:
                changedW = MIN_NAVIGATOR_WIDTH
            self.resizeFunc(changedW)
        else:
            event.Skip()

    def OnMotion(self, event):
        x,y = event.GetPosition()
        if x <= NAVMARGIN or self.dragStarted:
